...
)
{
  ...
  if (me == 0) {
    std::cout << "testTensorHash with nModes: " << nModes << "\n modeSizes: ";
  ...
  typedef pt::TensorHashMap<scalar_t,gno_t> hashmap_t;
  ...
  // Hash entries of sptensor
  size_t nNonZeros = sptensor->getLocalNumNonZeros();
  hashmap_t *hmap = new hashmap_t(nModes, nNonZeros);
  auto globalIndices = sptensor->getGlobalIndices();
  for (size_t i = 0; i < nNonZeros; i++)
    hmap->insert(subview(globalIndices, i, Kokkos::ALL),i);

  // Confirm a random tensor nonzero is in hashmap
  lno_t entry = rand() % sptensor->getLocalNumNonZeros();
  Kokkos::View<gno_t *> inds = subview(globalIndices, entry, Kokkos::ALL);
  if (!hmap->exists(inds)) {
    ierr++;
    
    std::cout << me << ":  Error in hashmap: local entry (";
    for (pt::mode_t m = 0; m < nModes-1; m++) {
      std::cout << inds(m) << ",";
    }
    std::cout << inds(nModes-1) << ") not found in map" << std::endl;
  }
  
  // Confirm entry outside the bounds of tensor is not in hashmap
  // (use entry outside of range of tensor)
  for (pt::mode_t m = 0; m < nModes-1; m++) {
      inds(m) = modeSizes[m];
  }
  if (hmap->exists(inds)) {
    ...
    std::cout << inds(nModes-1) << ") is found in map but shouldn't be" << std::endl;
  }
  
  // Print histogram
  if (verbosity) hmap->print_histogram(std::cout);
...
int main(int narg, char *arg[])
{
  ...
  try
  {
    ...
    ierr += testTensorHash(nModes, modeSizes, dist, comm);
  }
  catch (std::exception &e) {
