...
int testKtensor<scalar_t>::run()
{
  ...
  ktensorFromDefaultMaps = new ktensor_t(rank, maps, comm);

  ierr += checkKtensor("ktensorFromDefaultMaps", ktensorFromDefaultMaps, maps);

  // Test a ktensor created from sizes 
  ktensorFromSizes = new ktensor_t(rank, modeSizes, comm);

  ierr += checkKtensor("ktensorFromSizes", ktensorFromSizes, maps);
  ...
  ktensorFromCyclicMaps = new ktensor_t(rank, maps, comm);

  ierr += checkKtensor("ktensorFromCyclicMaps", ktensorFromCyclicMaps, maps);
  
  for (pt::mode_t m = 0; m < nModes; m++) { delete maps[m]; maps[m] = NULL; }
